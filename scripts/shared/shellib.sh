...
execsql() {
  eech "${BPur}>=== Executing with '${1}': ${3}"
  psql -v ON_ERROR_STOP=1 --username "${1}" -f ${3} <<EOT
    ${2}
EOT
}

execondb() {
  eech "${BPur}>=== Executing with '${1} on ${3}': ${4}"
  psql -v ON_ERROR_STOP=1 --username "${1}" --dbname ${3} -f ${4} <<EOT
    ${2}
EOT
}